...
ETHERSCAN_SECRET_NAME="${ETHERSCAN_SECRET_NAME:-vartovii-etherscan-api-key}"
MONGODB_DATABASE="${MONGODB_DATABASE:-vartovii}"
MONGODB_MCP_ENABLED="${MONGODB_MCP_ENABLED:-true}"
LIVE_WALLET_ADDRESS="${LIVE_WALLET_ADDRESS:-0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045}"
...
upsert_secret "${ETHERSCAN_SECRET_NAME}" "${ETHERSCAN_API_KEY:-}"
...
if [[ -n "${ETHERSCAN_API_KEY:-}" ]]; then
    gcloud secrets add-iam-policy-binding "${ETHERSCAN_SECRET_NAME}" \
        --member="serviceAccount:${SERVICE_ACCOUNT}" \
        --role="roles/secretmanager.secretAccessor" \
        --project="${PROJECT_ID}" \
        --quiet >/dev/null
...
if [[ -n "${ETHERSCAN_API_KEY:-}" ]]; then
    SECRET_ARGS+=("ETHERSCAN_API_KEY=${ETHERSCAN_SECRET_NAME}:latest")
fi
